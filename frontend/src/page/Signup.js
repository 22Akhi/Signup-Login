...
<input type= {"file"} id = "profileImage" className = "hidden"  accept = "image/*" onChange = {handleUploadProfileImage}/>
      ...
      <form className = "w-full py-3 flex flex-col" onSubmit = {handleSubmit }>
        ...
        <input type = {"text"} id = "firstName" name = "firstName" className = "mt-1 w-full bg-slate-200 px-2 mb-2 py-1 rounded focus-within:outline-blue-300" value= {data.firstName} onChange = {HandleOnChange}/>

        <label htmlFor = 'lastName' >Last Name</label>
        <input type = {"text"} id = "lastName" name = "lastName" className = "mt-1 w-full mb-2 bg-slate-200 px-2 py-1 rounded focus-within:outline-blue-300"  value = {data.lastName} onChange = {HandleOnChange}/>

        <label htmlFor = "email">Email</label>
        <input type = {"email"} id= "email" name = "email" className= "mt-1 w-full mb-2 bg-slate-200 px-2 py-1 rounded focus-within:outline-blue-300" value= {data.email} onChange = {HandleOnChange}/>
      ...
      <button  className = "w-full max-w-[150px] m-auto w-full bg-red-500 hover:bg-red-600 cursor-pointer text-white text-xl font-medium text-center py-1 rounded-full mt-4 ">Sign up</button>
      </form>
